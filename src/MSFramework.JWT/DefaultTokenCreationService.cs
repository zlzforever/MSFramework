...
namespace MSFramework.IdentityServer4
{
	...
	public class DefaultTokenCreationService : ITokenCreationService
	{
		...
		private readonly ISigningCredentialProvider _signingCredentialProvider;
		...
		/// <param name="signingCredentialProvider"></param>
		public DefaultTokenCreationService(
			ISystemClock clock,
			IdentityServerOptions options,
			ILogger<DefaultTokenCreationService> logger, ISigningCredentialProvider signingCredentialProvider)
		{
			Clock = clock;
			Options = options;
			Logger = logger;
			_signingCredentialProvider = signingCredentialProvider;
		}
		...
		{
			var credential = _signingCredentialProvider.Get();
			...
			{
				if (!string.IsNullOrWhiteSpace(Options.AccessTokenJwtType))
				{
